...
export async function zipFilesAsync(files: Record<string, ZipFile>, level: CompressionLevel = 6): Promise<Uint8Array> {
  const entries = Object.entries(files).map(([name, value]): ZipWorkerEntry => {
    ...
    return { name, bytes: buffer, mtime: options?.mtime?.getTime(), level: options?.level ?? level }
  })
...
function zipFilesInWorker(entries: ZipWorkerEntry[]): Promise<Uint8Array> {
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (event: MessageEvent<ZipWorkerResponse>) => {
      ...
      if (!response?.ok || !response.buffer) finish(() => reject(new Error(response?.error || 'ZIP Worker 压缩失败')))
      else finish(() => resolve(new Uint8Array(response.buffer!)))
    }

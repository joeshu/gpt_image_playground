...
describe('China Unicom semantic golden fixture', () => {
  it('contains native text, chart geometry, and only local image fallbacks', () => {
    ...
    expect(spec.elements.filter((element) => element.type === 'image').every((element) => element.box.width < 0.5 && element.box.height < 0.5)).toBe(true)
    expect(spec.elements.some((element) => element.type === 'image' && element.classification === 'imagegen_asset')).toBe(true)
    expect(spec.elements.filter((element) => element.type === 'image').every((element) => element.classification !== 'source_crop' || element.sourceExact === true)).toBe(true)
    expect(spec.elements.filter((element) => element.type === 'image' && element.classification === 'imagegen_asset').every((element) => Boolean(element.assetId && element.assetPrompt))).toBe(true)

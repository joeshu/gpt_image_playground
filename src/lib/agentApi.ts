...
export function sanitizeAgentText(text: string) {
  return text
    .replace(/\b(?:turn\s*\d+\s*search\s*\d+|search\s*\d+|cite)\b/gi, '')
    .replace(/[ \t]{2,}/g, ' ')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
...
async function parseAgentStreamResponse(
  ...
  const text = extractText(payload) || sanitizeAgentText(streamedText)
  return {
...
export async function callAgentResponsesApi(opts: {
  ...
  const timeoutId = setTimeout(() => controller.abort(new DOMException('请求超时', 'TimeoutError')), profile.timeout * 1000)
...
export async function callPptxSemanticAnalysisApi(opts: {
  ...
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  const controller = new AbortController()
  const semanticTimeoutSeconds = Math.max(900, profile.timeout)
  const timeoutId = setTimeout(() => controller.abort(new DOMException('PPTX 语义分析超时（已等待至少 15 分钟）', 'TimeoutError')), semanticTimeoutSeconds * 1000)
  const abortFromCaller = () => controller.abort()
  if (signal?.aborted) controller.abort()
  signal?.addEventListener('abort', abortFromCaller, { once: true })

        ...
        <div ref={cardRef} className={`min-w-0 overflow-hidden bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl border border-white/50 dark:border-white/[0.08] shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-2xl sm:rounded-3xl p-3 sm:p-4 ring-1 ring-black/5 dark:ring-white/10${promptExpanded ? ' flex min-h-0 flex-1 flex-col' : ''}`}>
          ...
          <div className={`relative grid${promptExpanded ? ' min-h-0 flex-1' : ''}`}>
            ...
            <div
              ...
              aria-label={promptPlaceholder}
               className={`col-start-1 row-start-1 min-h-[42px] w-full overflow-hidden ios-rounded-scroll-fix whitespace-pre-wrap break-words rounded-2xl border border-gray-200/60 bg-white/50 pl-4 pr-10 py-3 text-base leading-relaxed shadow-sm outline-none transition-[border-color,box-shadow] duration-200 focus:ring-1 focus:ring-blue-300/40 sm:text-sm dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-100 dark:focus:ring-blue-500/30${promptExpanded ? ' !h-full !overflow-y-auto' : ''}`}
            />
            {prompt.length === 0 && (
               <div className={`prompt-placeholder col-start-1 row-start-1 pointer-events-none pl-4 pr-10 py-3 text-base leading-relaxed text-gray-400 sm:text-sm dark:text-gray-500${
          ...
          <div className="mt-3">
            ...
            <div className="sm:hidden flex flex-col gap-2">
              <div className={`collapse-section${mobileCollapsed ? ' collapsed' : ''}`}>
                <div className="collapse-inner">
                  {renderParams('mobile-param-grid grid-cols-2 gap-y-3 pb-1')}
                  <div className="h-2" />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <button
                    onClick={() => {
                     ...
                     className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl transition-all shadow-sm ${
                      atImageLimit
                        ? 'bg-gray-200 dark:bg-white/[0.04] text-gray-300 dark:text-gray-500 cursor-not-allowed'
                ...
                >
                  ...
                  <button
                    ...
                    disabled={activeAgentIsRunning ? false : hasSubmitApiConfig ? !canSubmit : false}
                    aria-label={submitButtonAriaLabel}
                     className={`min-h-11 w-full flex items-center justify-center gap-2 rounded-xl text-sm font-medium transition-all shadow-sm ${
                      activeAgentIsRunning
                        ? 'bg-red-500 text-white hover:bg-red-600'
                        : !hasSubmitApiConfig
                        ? 'bg-gray-300 dark:bg-white/[0.06] text-white cursor-pointer'

    ...
    >
                ...
                <div className={`flex shrink-0 items-center gap-1 overflow-hidden transition-all duration-150 ${agentEditingConversationId === item.id ? 'w-6 opacity-100' : `group-hover:w-[4.5rem] group-hover:opacity-100 group-focus-within:w-[4.5rem] group-focus-within:opacity-100 ${conversationActionsId === item.id ? 'w-[4.5rem] opacity-100' : 'w-0 opacity-0'}`}`}>
                  {agentEditingConversationId === item.id ? (
                    ...
                    <>
                      ...
                      <AgentActionButton agentDeleteAction tooltip="删除" className="p-1.5 text-gray-400 hover:text-red-500" onClick={(e) => { e.stopPropagation(); handleDeleteConversation(item.id) }}>
      ...
      <section className="relative flex min-w-0 w-full max-w-full flex-1 flex-col">
        ...
        <div 
          ...
          className="min-w-0 w-full max-w-full flex-1 space-y-4 overflow-visible pb-[calc(var(--input-bar-clearance,12rem)+1.5rem)] px-1 lg:px-4 lg:pt-14"
                ...
                return (
                  <div key={message.id} className={`flex w-full mb-6 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
                    ...
                    {round?.status === 'error' && isAssistant && message.content.startsWith('请求失败：') ? (
                      ...
                      <div data-selectable-text className={`min-w-0 max-w-full overflow-hidden text-[13px] leading-relaxed text-gray-800 dark:text-gray-100 ${!isAssistant ? 'select-text' : ''}`}>
                        {isAssistant ? (
                          <>
                      ...
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        {isAssistant ? (
                          <>
                             ...
                             <AgentActionButton agentDeleteAction tooltip="删除消息" className="flex h-10 w-10 items-center justify-center hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors" onClick={() => {

...
- **高级创作工作台**：按项目集中管理品牌资产、固定事实、必保留原文、禁改项、Logo 使用规则、风格锁定、系列一致性和批量变量；事实、文案、比例、构图、风格可分别锁定，规则可一次应用到画廊或 Agent。
- **系列参考板与一致性锚点**：绑定系列参考图，结构化锁定主体、身份/外观、镜头/视角、背景/环境和构图/版式，并自动带入提示词、批量队列与复现快照。
- **可变批量队列**：提示词支持 `{{变量名}}` 插槽，建立批量任务前自动检查未定义变量、重名变量和空值；每个组合生成独立的变量替换提示词，失败仍停在当前项并支持重试。
- **创作复现快照**：保存提示词、项目规则、参数、参考图、遮罩和 API 配置标识，支持本机一键复现、导入与导出；快照不包含 API Key 或图片原始数据。
